...
(function() {
    function formatTime(time, type) {
        if ( type === 'time-ago' ) {
            ...
            if (time.isToday()) {
                ...
                if ( time < (2).days().fromNow()) {
                    return 'Tomorrow';
                } else if (time < (7).days().fromNow()) {
                    return time.strftime('%A');
                } else if (time.isThisYear()) {
                    return time.strftime('%b %e');
                } else {
    ...
    Viking.View.Helpers.localTime = function (time, format, options) {
        ...
        if (now === true) {
            options['data-local'] = 'now';
        } else {
            options['data-local'] = 'time';
            options['data-localized'] = 'true';
        }
    ...
    Viking.View.updateTimeTags = function () {
        _.each(document.querySelectorAll("time[data-local]:not([data-localized])"), function (element) {
            var format = element.getAttribute("data-format"),
                local = element.getAttribute("data-local"),
                time = new Date(Date.parse(element.getAttribute("datetime")));
        
            if (local === 'now') {
                time = new Date();
                element.setAttribute("title", time.strftime("%B %e, %Y at %l:%M%P %Z"));
